...
describe('email sign-up persists a real account', () => {
  ...
  it('rejects duplicate email or handle and bad input', async () => {
    ...
    // Any existing row refuses a duplicate, verified or not: the recovery path
    // for a half-finished signup is sign-in or password reset, never a second
    // registration (see account-lifecycle.test.ts).
